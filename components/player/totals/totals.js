  ...
  methods: {
    ...
    radarTimer(){
      this.data.radarTimer=setInterval(()=>{
        ...
        if (this.data.playersTotals20 && this.data.playersTotals100 && this.data.playersTotalsAll || i > MAX_RETRY_TIMES){
          //关闭定时器
    ...
    lineTimer(){
      let i = 0;
      this.lineintervalTimer=setInterval(()=>{
        //折线图需要playersRatings接口数据
        if (this.data.playersRatings || i > MAX_RETRY_TIMES){
          //关闭定时器
          clearInterval(this.lineintervalTimer)
          //call子组件
          this.callLine()
        }
        ++i;
